...
void config_save (const gchar *privcfgpath)
{
  ...
  gnome_config_set_bool ("geometry/numrows_follows_panel", config.numrows_follows_panel);
  gnome_config_set_int ("geometry/tasklist_width", config.tasklist_width);
...
void config_load (const gchar *privcfgpath)
{
  ...
  config.tasklist_width = gnome_config_get_int("geometry/tasklist_width=350");
  gnome_config_pop_prefix ();
}
