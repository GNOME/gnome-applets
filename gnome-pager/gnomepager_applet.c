...
static void init_applet_gui(int horizontal);
static void desktop_draw(int i);

static void set_task_info_to_button(Task * t);

static void populate_tasks(int just_popbox);

static void create_popbox(void);
static int showpop_cb(GtkWidget * widget, gpointer data);

static GdkFilterReturn cb_filter_intercept(GdkXEvent * gdk_xevent,
					   GdkEvent * event, gpointer data);



static GtkWidget *applet = NULL;
static GtkWidget *popbox = NULL;
static GtkWidget *popbox_q = NULL;
static GtkWidget **blists = NULL;
static GtkWidget **flists = NULL;
static int blists_num = 0;
static GList *tasks = NULL;
static PanelOrientType applet_orient = -1;
static int current_desk = 0;
static int num_desk = 0;
static char *desk_name[32];
static GtkWidget *desk_widget[32];
static GList *task_widgets = NULL;
static GList *task_dest = NULL;
static GtkWidget *hold_box = NULL;
static GtkWidget *main_box = NULL;
static GtkWidget *task_table = NULL;
static GtkWidget *prop_button = NULL;
static GtkWidget *arrow_button = NULL;
static GdkPixmap *p_1 = NULL, *p_2 = NULL, *p_3 = NULL;
static GdkPixmap *m_1 = NULL, *m_2 = NULL, *m_3 = NULL;
static GtkTooltips *ptooltips = NULL;
static GtkTooltips *ttooltips = NULL;

/*needed for gdk warnings*/
extern int gdk_error_warnings;

typedef struct _Config Config;
struct _Config {
	int reverse_order;		/*bool*/
	int pager_rows;
	int pager_size;			/*bool*/
	int ptooltips;			/*bool*/ /*pager tooltips*/
	int ttooltips;			/*bool*/ /*tasklist tooltips*/
	int tasks_all;			/*bool*/
	int minimized_tasks;		/*bool*/
	int minimized_tasks_all;	/*bool*/
	int minimized_tasks_only;	/*bool*/
	int task_rows_h;
	int task_rows_v;
	int max_task_width;
	int max_task_vwidth;
...
static Config config;	/*this is the actual current configuration */
static Config o_config;	/*temporary configuration used for properties */

static int area_w = 1;
static int area_h = 1;
static int area_x = 0;
static int area_y = 0;
static char tasks_changed = 0;
...
static void 
cb_check(GtkWidget * widget, int * the_data)
{
	GtkWidget *prop;
...
static void 
cb_adj(GtkAdjustment * adj, int * the_data)
{
	GtkWidget *prop;
...
cb_prop_apply(GtkWidget * widget, int page, gpointer data)
{
	...
	if(config.ttooltips)
		gtk_tooltips_enable(ttooltips);
	else
		gtk_tooltips_disable(ttooltips);
	if(config.ptooltips)
		gtk_tooltips_enable(ptooltips);
	else
		gtk_tooltips_disable(ptooltips);
}
...
static GtkWidget *
add_spin_button(GtkWidget *box, GtkWidget *prop, char *label,
		int def, int min, int max, int step, int page,
		gpointer change)
{
	GtkAdjustment *adj;
	GtkWidget *hbox, *w;

	adj = (GtkAdjustment *) gtk_adjustment_new((gfloat) def,
						   min, max, step, page, page);
...
static GtkWidget *
add_check_button(GtkWidget *box, GtkWidget *prop, char *label,
		 int def, gpointer change)
{
	GtkWidget *w;
	w = gtk_check_button_new_with_label(label);
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(w), def);
	gtk_signal_connect(GTK_OBJECT(w), "toggled",
			GTK_SIGNAL_FUNC(cb_check), change);
...
cb_applet_properties(AppletWidget * widget, gpointer data)
{
	...
	GtkWidget *hbox, *vbox, *vbox1, *frame, *w, *sh;
	GtkWidget *rb1, *rb2;
	...
	sh = add_check_button(vbox, prop,_("Show pager"),
			      config.show_pager, &o_config.show_pager);

	w = add_check_button(vbox, prop,_("Place pagers after tasklist"),
			     config.reverse_order, &o_config.reverse_order);
	gtk_signal_connect(GTK_OBJECT(sh), "toggled",
			   GTK_SIGNAL_FUNC(cb_check_enable), w);
	cb_check_enable(sh, w);

	w = add_check_button(vbox, prop,_("Use small pagers"),
			     config.pager_size, &o_config.pager_size);
	gtk_signal_connect(GTK_OBJECT(sh), "toggled",
			   GTK_SIGNAL_FUNC(cb_check_enable), w);
	cb_check_enable(sh, w);

	w = add_check_button(vbox, prop,_("Enable pager tooltips"),
			     config.ptooltips, &o_config.ptooltips);
	gtk_signal_connect(GTK_OBJECT(sh), "toggled",
			   GTK_SIGNAL_FUNC(cb_check_enable), w);
	cb_check_enable(sh, w);

	add_spin_button(vbox,prop,_("Rows of pagers"),
			config.pager_rows, 1, 8, 1, 1, &o_config.pager_rows);

	add_spin_button(vbox,prop,_("Width of small pagers"),
			config.pager_w_0, 1, 100, 1, 1, &o_config.pager_w_0);

	add_spin_button(vbox,prop,_("Height of small pagers"),
			config.pager_h_0, 1, 100, 1, 1, &o_config.pager_h_0);

	add_spin_button(vbox,prop,_("Width of large pagers"),
			config.pager_w_1, 1, 100, 1, 1, &o_config.pager_w_1);

	w = add_spin_button(vbox,prop,_("Height of large pagers"),
			    config.pager_h_1, 1, 100, 1, 1,
			    &o_config.pager_h_1);
	...
	add_check_button(vbox, prop,_("Show task list button"),
			 config.show_arrow, &o_config.show_arrow);

	sh = add_check_button(vbox, prop,_("Show task list"),
			      config.show_tasks, &o_config.show_tasks);

	w = add_check_button(vbox, prop,_("Show button icons"),
			     config.show_icons, &o_config.show_icons);
	...
	rb1 = w = gtk_radio_button_new_with_label(NULL, _("Show all tasks"));
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(w), config.minimized_tasks);
	gtk_object_set_data(GTK_OBJECT(w), "prop", prop);
	gtk_box_pack_start(GTK_BOX(vbox1), w, FALSE, FALSE, 0);

	w = gtk_radio_button_new_with_label_from_widget(GTK_RADIO_BUTTON(rb1),
							_("Show normal tasks only"));
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(w),
				     !config.minimized_tasks && !config.minimized_tasks_only);
	gtk_box_pack_start(GTK_BOX(vbox1), w, FALSE, FALSE, 0);

	rb2 = w = gtk_radio_button_new_with_label_from_widget(GTK_RADIO_BUTTON(rb1),
							      _("Show minimized tasks only"));
	gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(w), config.minimized_tasks_only);
	gtk_object_set_data(GTK_OBJECT(w), "prop", prop);
	gtk_box_pack_start(GTK_BOX(vbox1), w, FALSE, FALSE, 0);
	...
	add_check_button(vbox1, prop,_("Show all tasks on all desktops"),
			 config.tasks_all, &o_config.tasks_all);

	add_check_button(vbox1, prop,_("Show minimized tasks on all desktops"),
			 config.minimized_tasks_all,
			 &o_config.minimized_tasks_all);

	add_check_button(vbox1, prop,_("Enable task list tooltips"),
			 config.ttooltips, &o_config.ttooltips);
	...
	add_spin_button(vbox1,prop,_("Maximum width of horizontal task list"),
			...
			20, (gfloat) gdk_screen_width(), 10, 50,
			&o_config.max_task_width);

	add_spin_button(vbox1,prop,_("Maximum width of vertical task list"),
			config.max_task_vwidth,
			4, (gfloat) gdk_screen_width(), 4, 16,
			&o_config.max_task_vwidth);

	add_spin_button(vbox1,prop,_("Number of rows of horizontal tasks"),
			config.task_rows_h,
			1,8,1,4,
			&o_config.task_rows_h);

	add_spin_button(vbox1,prop,_("Number of vertical columns of tasks"),
			config.task_rows_v,
			1,4,1,2,
			&o_config.task_rows_v);
...
static void *
util_get_atom(Window win, char * atom, Atom type, int * size)
{
...
showpop_cb(GtkWidget * widget, gpointer data)
{
	...
	/*XXX: return false even though clicked is a void signal, but all
	  the _event's need a return value */
	return FALSE;
}
...
task_get_info(Task * t)
{
	...
	XGetGeometry(GDK_DISPLAY(), t->frame,
		     &ret, &size, &size, &w, &h, &d, &d);
	t->w = (int) w;
	t->h = (int) h;
...
/*  gdk_window_set_user_data(t->gdkwin, t->dummy); */
	...
	if (!t->gdkwin ||
	    gdk_window_get_type(t->gdkwin) == GDK_WINDOW_FOREIGN) {
		XSelectInput(GDK_DISPLAY(), win,
			     PropertyChangeMask | FocusChangeMask |
			     StructureNotifyMask);
	} else {
		/*if the window is ours don't make it select out some events
		  only! */
		XWindowAttributes attr;
		XGetWindowAttributes(GDK_DISPLAY(), win, &attr);
		/*FIXME: why does PropertyChangeMask generate Gdk warnings
		  when we close our own properties dialog*/
		XSelectInput(GDK_DISPLAY(), win,
...
tasks_match(Window * win, guint num)
{
	...
	while (p1) {
		...
		if (t) {
			...
			if (!there) {
				tasks_changed = 1;
				/*XXX:although it will work in this case,
				  removing stuff from a list while we are
				  going through it is nuts and asking for
				  trouble */
				task_delete(t);
			}
		}
		       ...
		       const char *globcfgpath)
{
	...
	gnome_config_set_bool("stuff/ptooltips", config.ptooltips);
	gnome_config_set_bool("stuff/ttooltips", config.ttooltips);
	gnome_config_set_int("stuff/tasks_all", config.tasks_all);
	gnome_config_set_int("stuff/minimized_tasks", config.minimized_tasks);
	gnome_config_set_int("stuff/minimized_tasks_all",
			     config.minimized_tasks_all);
	gnome_config_set_int("stuff/minimized_tasks_only",
...
desktop_cb_button_down(GtkWidget * widget, GdkEventButton * event)
{
	...
	if (button == 1) {
		...
		x = (((int) event->x * area_w) - 2) / w;
		y = (((int) event->y * area_h) - 2) / h;
		if (x >= area_w)
			x = area_w - 1;
		if (x < 0)
			x = 0;
...
actual_redraw(gpointer data)
{
	...
	GtkStyle *s;
	GdkWindow *win;
	int sel, sw, sh, w, h, desk, x, y, ww, hh;
	Task *t;
	GList *p, *stacking;
	GdkBitmap *bm;
	GdkGC *gc;
...
desktop_cb_redraw(GtkWidget * widget, gpointer data)
{
	...
	current_timeout = gtk_timeout_add(200,
					  actual_redraw,
					  widget);
	gtk_object_set_data(GTK_OBJECT(widget), "timeout",
			    GINT_TO_POINTER(current_timeout));
	return FALSE;
}
...
static GtkWidget *
make_desktop_pane(int desktop, int width, int height)
{
	GtkWidget *area, *tip;
	char *s;
	...
	s = desk_name[desktop];
	if (s)
		gtk_tooltips_set_tip(ptooltips, area, s, NULL);
	return area;
}
...
populate_tasks(int just_popbox)
{
	Task *t;
	GList *p;
	GtkWidget *hbox, *button, *icon1, *icon2, *icon3, *label, *menu,
	*tip;
#ifdef ANIMATION
	GtkWidget *f1, *f2, *f3, *f4;
#endif
	int n_rows, n_cols, num, i, j, k;
	...
	while (p) {
		...
		if ((((t->desktop == current_desk)
			...
			gtk_tooltips_set_tip(GTK_TOOLTIPS(ttooltips),
					     button, t->name, NULL);

			gtk_signal_connect(GTK_OBJECT(button),
					   "button_press_event",
					   GTK_SIGNAL_FUNC(popup_button_pressed),
					   t);
...
main(int argc, char *argv[])
{
	...
	if (!gnome_win_hints_wm_exists()) {
		...
		l = gtk_label_new(_("You are not running a GNOME Compliant "
				    "Window Manager.\n"
				    "Please run a GNOME Compliant "
				    "Window Manager.\n"
				    "For Example:\n"
	...
	config.ptooltips = gnome_config_get_bool("stuff/ptooltips=true");
	config.ttooltips = gnome_config_get_bool("stuff/ttooltips=true");
	config.tasks_all = gnome_config_get_int("stuff/tasks_all=0");
	config.minimized_tasks =
		gnome_config_get_int("stuff/minimized_tasks=1");
	config.minimized_tasks_all =
		gnome_config_get_int("stuff/minimized_tasks_all=0");
	config.minimized_tasks_only =
		gnome_config_get_int("stuff/minimized_tasks_only=0");
	config.task_rows_h = gnome_config_get_int("stuff/task_rows_h=2");
	config.task_rows_v = gnome_config_get_int("stuff/task_rows_v=1");
	config.max_task_width =
		gnome_config_get_int("stuff/max_task_width=400");
	config.max_task_vwidth =
		gnome_config_get_int("stuff/max_task_vwidth=48");
