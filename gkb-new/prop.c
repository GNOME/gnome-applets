 ...
 **/
static void
gkb_prop_label_at (GtkWidget *table, gint row, gint col, const gchar *label_text)
{
  GtkWidget *label;
 ...
 **/
static void
gkb_prop_appeareance_changed (GtkWidget *menu_item, GkbPropertyBoxInfo * pbi)
{
  gchar *text;
  ...
  pbi->appeareance = gkb_util_get_appeareance_from_text (text);
			 ...
			 GkbPropertyBoxInfo *pbi, gint initval)
{
  ...
  for (; list != NULL; list = list->next) {
    ...
    gtk_signal_connect (GTK_OBJECT (menu_item), "activate",
			function, pbi);
    gtk_object_set_data (GTK_OBJECT (menu_item), GKB_MENU_ITEM_TEXT, list->data);
  }
...
gkb_prop_create_property_box (GkbPropertyBoxInfo *pbi)
{
  ...
  gtk_signal_connect (GTK_OBJECT (propbox), "apply",
		      GTK_SIGNAL_FUNC (gkb_prop_apply_clicked), 
		      pbi);
...
properties_dialog (AppletWidget * applet)
{
  ...
  gtk_signal_connect (GTK_OBJECT (pbi->box), "destroy",
		      GTK_SIGNAL_FUNC (gkb_prop_box_destroy),
		      pbi);
