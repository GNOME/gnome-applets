...
static void CappletAddAvailablePluginFunc( const char *fullPath,
                                           ...
                                           GSwitchItPluginsCapplet * gswic )
{
  ...
  if( NULL !=
      g_slist_find_custom( gswic->appletConfig.enabledPlugins, fullPath,
                           ( GCompareFunc ) strcmp ) )
    return;
...
static void CappletFillAvailablePluginList( GtkTreeView *
                                            ...
                                            GSwitchItPluginsCapplet * gswic )
{
  ...
  g_hash_table_foreach( allPluginRecs,
                        ( GHFunc ) CappletAddAvailablePluginFunc, gswic );
  g_object_set_data( G_OBJECT( gswic->capplet ),
                     "gswitchit_plugins_add.availablePluginsModel", NULL );
  pluginPathNode = g_slist_next( pluginPathNode );
}

static void CappletAvailablePluginsSelectionChanged( GtkTreeSelection *
                                                     selection,
...
void CappletEnablePlugin( GtkWidget * btnAdd,
                          GSwitchItPluginsCapplet * gswic )
{
  GladeXML *data = glade_xml_new( GLADE_DIR "/gswitchit-plugins.glade", "gswitchit_plugins_add", NULL );        // default domain!
  GtkWidget *popup = glade_xml_get_widget( data, "gswitchit_plugins_add" );
  GtkWidget *availablePluginsList;
  ...
  gtk_tree_view_set_model( GTK_TREE_VIEW
                           ( availablePluginsList ), availablePluginsModel );
  gtk_tree_view_append_column( GTK_TREE_VIEW
                               ( availablePluginsList ), column );
  selection =
  ...
  g_signal_connect( G_OBJECT( selection ), "changed",
                    G_CALLBACK
                    ( CappletAvailablePluginsSelectionChanged ), gswic );
  g_object_set_data( G_OBJECT( gswic->capplet ),
                     "gswitchit_plugins_add.btnOK",
  ...
  if( response == GTK_RESPONSE_OK )
  {
    ...
    if( fullPath != NULL )
    {
      GSwitchItPluginManagerEnablePlugin( &gswic->pluginManager,
