...
{
  GtkWidget *activePlugins = CappletGetGladeWidget( gswic, "activePlugins" );
  ...
  while( pluginPathNode != NULL )
  {
    ...
    const GSwitchItPlugin *plugin =
      GSwitchItPluginManagerGetPlugin( &gswic->pluginManager, fullPath );
    if( plugin != NULL )
    {
      gtk_list_store_append( activePluginsModel, &iter );
      gtk_list_store_set( activePluginsModel, &iter,
                          NAME_COLUMN, plugin->name, FULLPATH_COLUMN,
                          fullPath, -1 );
    }
...
static void CappletActivePluginsSelectionChanged( GtkTreeSelection *
                                                  selection,
                                                  ...
                                                  gswic )
{
  ...
  GtkWidget *btnProperties = CappletGetGladeWidget( gswic, "btnProperties" );
  ...
  if( gtk_tree_selection_get_selected( selection, NULL, &selectedIter ) )
  {
    ...
    if( plugin != NULL )
    {
      hasConfigurationUi = ( plugin->configurePropertiesCallback != NULL );
      gtk_label_set_text( GTK_LABEL( lblDescription ), plugin->description );
    }
    g_free( fullPath );
...
static void CappletPromotePlugin( GtkWidget * btnUp,
                                  GSwitchItPluginsCapplet * gswic )
{
  char *fullPath = CappletGetSelectedActivePluginPath( gswic );
  if( fullPath != NULL )
  {
    GSwitchItPluginManagerPromotePlugin( &gswic->pluginManager,
                                         gswic->appletConfig.enabledPlugins,
                                         fullPath );
    g_free( fullPath );
...
static void CappletDemotePlugin( GtkWidget * btnUp,
                                 GSwitchItPluginsCapplet * gswic )
{
  char *fullPath = CappletGetSelectedActivePluginPath( gswic );
  if( fullPath != NULL )
...
static void CappletDisablePlugin( GtkWidget * btnRemove,
                                  GSwitchItPluginsCapplet * gswic )
{
  char *fullPath = CappletGetSelectedActivePluginPath( gswic );
  if( fullPath != NULL )
...
static void CappletConfigurePlugin( GtkWidget * btnRemove,
                                    GSwitchItPluginsCapplet * gswic )
{
  char *fullPath = CappletGetSelectedActivePluginPath( gswic );
  if( fullPath != NULL )
  {
    GSwitchItPluginManagerConfigurePlugin( &gswic->pluginManager,
                                           &gswic->pluginContainer,
                                           fullPath,
...
static void CappletSetup( GSwitchItPluginsCapplet * gswic )
{
  GladeXML *data;
  GtkWidget *capplet;
  ...
  GtkTreeViewColumn *column =
    gtk_tree_view_column_new_with_attributes( NULL, renderer, "text", 0,
                                              NULL );
  GtkTreeSelection *selection;
  const char *iconFile;
  glade_gnome_init(  );
