...
static gint panel_menu_path_remove_cb(GtkWidget *widget, GdkEventKey *event, PanelMenuPath *path);
static void change_path_cb(GtkWidget *widget, PanelMenuEntry *entry);
static GtkWidget *panel_menu_path_edit_dialog_new(gchar *title, gchar *value, GtkWidget **entry);
...
panel_menu_path_new(PanelMenu *parent, gchar *menu_path)
{
    ...
    gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(path->checkitem), TRUE);
    g_signal_connect(G_OBJECT(path->checkitem), "toggled", G_CALLBACK(panel_menu_common_set_visibility), path->path);
    gtk_widget_show(path->checkitem);
    path->menu = gtk_menu_new();
    tearoff = gtk_tearoff_menu_item_new();
    gtk_menu_shell_append(GTK_MENU_SHELL(path->menu), tearoff);
    gtk_widget_show(tearoff);
    path->regenitem = gtk_image_menu_item_new_with_label(_("Regenerate Menus"));
    image = gtk_image_new_from_stock("gtk-refresh", GTK_ICON_SIZE_MENU);
    gtk_image_menu_item_set_image(GTK_IMAGE_MENU_ITEM(path->regenitem), image);
    gtk_widget_show(image);
    gtk_menu_shell_append(GTK_MENU_SHELL(path->menu), path->regenitem);
    g_signal_connect(G_OBJECT(path->regenitem), "activate", G_CALLBACK(regenerate_menus_cb), entry);
    gtk_widget_show(path->regenitem);
    sep = gtk_menu_item_new();
    gtk_menu_shell_append(GTK_MENU_SHELL(path->menu), sep);
    gtk_widget_show(sep);
    sep = gtk_menu_item_new();
    g_object_set_data(G_OBJECT(sep), "append-before", GINT_TO_POINTER(TRUE));
    gtk_menu_shell_append(GTK_MENU_SHELL(path->menu), sep);
    gtk_widget_show(sep);
    gtk_menu_item_set_submenu(GTK_MENU_ITEM(path->path), path->menu);
    GTK_MENU(path->menu)->parent_menu_item = path->path;
    g_signal_connect(G_OBJECT(path->menu), "key_press_event", G_CALLBACK(panel_menu_path_remove_cb), path);
    path->popup = panel_menu_common_construct_entry_popup(entry);
    item = gtk_image_menu_item_new_with_label(_("Change Path..."));
    image = gtk_image_new_from_stock(GTK_STOCK_CONVERT, GTK_ICON_SIZE_MENU);
    gtk_image_menu_item_set_image(GTK_IMAGE_MENU_ITEM(item), image);
    gtk_widget_show(image);
    gtk_menu_shell_append(GTK_MENU_SHELL(path->popup), item);
    g_signal_connect(G_OBJECT(item), "activate", G_CALLBACK(change_path_cb), entry);
...
panel_menu_path_set_uri(PanelMenuEntry *entry, gchar *uri)
{
    ...
    if(path->base_path) g_free(path->base_path);
    full_path = panel_menu_common_build_full_path(uri, ".directory");
    item = panel_menu_desktop_item_load_uri((const char *)full_path, "Directory", TRUE);
    g_free(full_path);
    if(item)
    {
    ...
    gtk_label_set_text(GTK_LABEL(GTK_BIN(path->checkitem)->child), show_label);
    g_free(show_label);
    g_free(label);
...
regenerate_menus_cb(GtkWidget *menuitem, PanelMenuEntry *entry)
{
    ...
    g_list_free(list);
    panel_menu_path_load((const gchar *)path->base_path, GTK_MENU_SHELL(path->menu));
    if(path->paths_list)
    {
        ...
        for(cur = list; cur; cur = cur->next)
        {
            panel_menu_path_append_item(entry, (gchar *)cur->data);
            g_free(cur->data);
        }
        g_list_free(list);
    }
...
panel_menu_path_destroy(PanelMenuEntry *entry)
{
    ...
    if(path->base_path) g_free(path->base_path);
    if(path->items_list) g_list_free(path->items_list);
...
GtkWidget *
panel_menu_path_get_popup(PanelMenuEntry *entry)
{
    PanelMenuPath *path;
...
directory_load_cb(GnomeVFSAsyncHandle *handle, GnomeVFSResult result, GList *list, guint entries_read, GtkMenuShell *parent)
{
    ...
    for(iter = list, count = 0; iter && count < entries_read; iter = iter->next, count++)
    {
        ...
        {
            submenu = panel_menu_common_menu_from_path(finfo->name, subpath, parent, FALSE);
            panel_menu_path_load((const gchar *)subpath, GTK_MENU_SHELL(submenu));
        }
        else
        {
            if(!strcmp(".directory", finfo->name))
            {
                item = panel_menu_desktop_item_load_uri((const char *)subpath, "Directory", TRUE);
                if(item)
                {
                    ...
                    gtk_label_set_text(GTK_LABEL(GTK_BIN(GTK_MENU(parent)->parent_menu_item)->child), item->name);
                    if(GTK_IS_IMAGE_MENU_ITEM(GTK_MENU(parent)->parent_menu_item))
                    {
                        icon = panel_menu_desktop_item_find_icon(item->icon);
                        if(icon)
                        {
                            panel_menu_common_set_icon_scaled_from_file(GTK_MENU_ITEM(GTK_MENU(parent)->parent_menu_item), icon);
                            g_free(icon);
                        }
            ...
            {
                menuitem = panel_menu_common_menuitem_from_path(subpath, parent, FALSE);
            }
        }
        g_free(subpath);
    }
...
panel_menu_path_append_item(PanelMenuEntry *entry, gchar *uri)
{
    ...
    if(!strncmp(uri, "applications:", strlen("applications:")) || (!strncmp(uri, "file:", strlen("file:")) && strstr(uri, ".desktop")))
    {
        ...
        {
            if((menuitem = panel_menu_common_menuitem_from_path(uri, GTK_MENU_SHELL(path->menu), TRUE)))
                retval = TRUE;
        }
        else
        {
            submenu = panel_menu_common_menu_from_path(_("Programs"), uri, GTK_MENU_SHELL(path->menu), TRUE);
            menuitem = GTK_MENU(submenu)->parent_menu_item;
            panel_menu_path_load((const gchar *)uri, GTK_MENU_SHELL(submenu));
            retval = TRUE;
        }
        if(retval)
        {
            path->items_list = g_list_append(path->items_list, menuitem);
            path->paths_list = g_list_append(path->paths_list, g_strdup(uri));
        }
    }
    return(retval);
}
...
static gint
panel_menu_path_remove_cb(GtkWidget *widget, GdkEventKey *event, PanelMenuPath *path)
{
    GtkWidget *item;
    GList *cur;
    gchar *string;
    ...
    if(event->keyval == GDK_Delete)
    {
        ...
        for(cur = path->items_list; cur; cur = cur->next, counter++)
        {
            ...
            {
                path->items_list = g_list_remove(path->items_list, cur->data);
                temp = (gchar *)g_list_nth_data(path->paths_list, counter);
                g_print("removing %s from the paths list\n", temp);
                path->paths_list = g_list_remove(path->paths_list, temp);
...
panel_menu_path_new_with_dialog(PanelMenu *panel_menu)
{
    ...
    dialog = panel_menu_path_edit_dialog_new(_("Create menu path item..."), "applications:", &path_entry);
    ...
    if(response == GTK_RESPONSE_OK)
    {
        ...
        panel_menu_options_append_option(panel_menu_common_find_options(panel_menu), panel_menu_path_get_checkitem(entry));
...
static void
change_path_cb(GtkWidget *widget, PanelMenuEntry *entry)
{
    PanelMenuPath *path;
    GtkWidget *dialog;
    GtkWidget *path_entry;
    ...
    path = (PanelMenuPath *)entry->data;
    dialog = panel_menu_path_edit_dialog_new(_("Edit path item..."), path->base_path, &path_entry);
    gtk_widget_show(dialog);
    gtk_widget_grab_focus(path_entry);
...
static GtkWidget *
panel_menu_path_edit_dialog_new(gchar *title, gchar *value, GtkWidget **entry)
{
    GtkWidget *dialog;
    GtkWidget *box;
    GtkWidget *hbox;
