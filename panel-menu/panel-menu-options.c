...
panel_menu_options_new(PanelMenu *parent, const gchar *icon)
{
    ...
    gtk_menu_item_set_submenu(GTK_MENU_ITEM(options->options), options->menu);
    return(entry);
}

void
panel_menu_options_set_icon(PanelMenuEntry *entry, const gchar *icon, gint size)
{
    PanelMenuOptions *options;
    GdkPixbuf *pixbuf;
    GdkPixbuf *scaled;
    ...
    if(icon)
    {
        ...
        {
            if(entry->parent->icon_handle_image) g_free(entry->parent->icon_handle_image);
        ...
        if(pixbuf)
        {
            ...
            if(scaled)
            {
                ...
                gtk_image_menu_item_set_image(GTK_IMAGE_MENU_ITEM(options->options), image);
    ...
    else
    {
        ...
        gtk_image_menu_item_set_image(GTK_IMAGE_MENU_ITEM(options->options), NULL);
    }
}
...
void
panel_menu_options_insert_option(PanelMenuEntry *entry, GtkWidget *option, gint position)
{
    PanelMenuOptions *options;
    ...
    options = (PanelMenuOptions *)entry->data;
    gtk_menu_shell_insert(GTK_MENU_SHELL(options->menu), option, position);
    gtk_widget_show(option);
}
...
panel_menu_options_rescale(PanelMenuEntry *entry)
{
    ...
    options = (PanelMenuOptions *)entry->data;
    pixbuf = gdk_pixbuf_new_from_file(entry->parent->icon_handle_image, NULL);
    scaled = gdk_pixbuf_scale_simple(pixbuf, entry->parent->size - 8, entry->parent->size - 8, GDK_INTERP_BILINEAR);
    image = gtk_image_new_from_pixbuf(scaled);
    g_object_unref(G_OBJECT(pixbuf));
    gtk_image_menu_item_set_image(GTK_IMAGE_MENU_ITEM(options->options), image);
    gtk_widget_show(image);
