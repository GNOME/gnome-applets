...
static void applet_change_background(PanelApplet *applet, PanelAppletBackgroundType type, GdkColor *color, const gchar *pixmap, PanelMenu *panel_menu);
static void applet_size_allocate(GtkWidget *widget, GtkAllocation *allocation, PanelMenu *panel_menu);
...
static gboolean panel_menu_id_from_key(gchar *key, gchar **profile, gchar **applet);

static gboolean panel_menu_drag_motion(GtkWidget *widget, GdkDragContext *context, gint x, gint y, guint time, PanelMenu *panel_menu);
static void panel_menu_drag_leave(GtkWidget	*widget, GdkDragContext *context, guint time, PanelMenu *panel_menu);
static void panel_menu_drag_data_received(GtkWidget *widget, GdkDragContext *context, gint x, gint y, GtkSelectionData *selection_data, guint info, guint time, PanelMenu *panel_menu);
static gboolean panel_menu_pass_drop(PanelMenu *panel_menu, GnomeVFSURI *uri);
static gboolean panel_menu_check_entry_drop(PanelMenu *panel_menu, PanelMenuEntry *entry);
static void panel_menu_accept_entry_drop(PanelMenu *panel_menu, PanelMenuEntry *entry);

static gint applet_button_press_cb(GtkWidget *widget, GdkEventButton *event, gpointer data);
static gint applet_destroy_cb(GtkWidget *widget, PanelMenu *panel_menu);
static void applet_change_orientation_cb(PanelApplet *applet, PanelAppletOrient  orient, PanelMenu *panel_menu);
static void applet_change_size_cb(PanelApplet  *applet, gint size, PanelMenu *panel_menu);
static void applet_help_cb(BonoboUIComponent *uic, PanelMenu *panel_menu, const gchar *verbname);
static void applet_about_cb(BonoboUIComponent *uic, PanelMenu *panel_menu, const gchar *verbname);
...
static GtkTargetEntry drop_types[] = {
	...
	{ "application/panel-menu-entry", GTK_TARGET_SAME_APP, TARGET_PANEL_MENU_ENTRY }
};
...
panel_menu_construct_applet(PanelApplet *applet)
{
    ...
    panel_menu_id_from_key(key, &panel_menu->profile_id, &panel_menu->applet_id);
    g_free(key);

    panel_applet_add_preferences (applet, "/schemas/apps/panel-menu-applet/prefs", NULL);
    panel_menu_config_load_prefs(panel_menu);
    panel_menu->menubar = gtk_menu_bar_new();
    gtk_drag_dest_set(panel_menu->menubar, GTK_DEST_DEFAULT_ALL, drop_types, n_drop_types, GDK_ACTION_COPY | GDK_ACTION_LINK | GDK_ACTION_ASK);
    g_signal_connect(G_OBJECT(panel_menu->menubar), "drag_motion", G_CALLBACK(panel_menu_drag_motion), panel_menu);
    g_signal_connect(G_OBJECT(panel_menu->menubar), "drag_leave", G_CALLBACK(panel_menu_drag_leave), panel_menu);
    g_signal_connect(G_OBJECT(panel_menu->menubar), "drag_data_received", G_CALLBACK(panel_menu_drag_data_received), panel_menu);

    gtk_container_add(GTK_CONTAINER(panel_menu->applet), panel_menu->menubar);
    ...
    g_signal_connect(G_OBJECT(panel_menu->menubar),"button_press_event", G_CALLBACK(applet_button_press_cb), panel_menu->applet);
    g_signal_connect(G_OBJECT(panel_menu->menubar),"size_allocate", G_CALLBACK(applet_size_allocate), panel_menu);
    g_signal_connect(G_OBJECT(panel_menu->applet), "destroy", G_CALLBACK(applet_destroy_cb), panel_menu);

    g_signal_connect(G_OBJECT(panel_menu->applet), "change_background", G_CALLBACK(applet_change_background), panel_menu);
    g_signal_connect(G_OBJECT(panel_menu->applet), "change_orient", G_CALLBACK(applet_change_orientation_cb), panel_menu);
    g_signal_connect(G_OBJECT(panel_menu->applet), "change_size", G_CALLBACK(applet_change_size_cb), panel_menu);

    panel_applet_setup_menu(panel_menu->applet, applet_menu_xml, applet_menu_verbs, panel_menu);

    return(TRUE);
}

static void
applet_change_background(PanelApplet *applet, PanelAppletBackgroundType type, GdkColor *color, const gchar *pixmap, PanelMenu *panel_menu)
{
    ...
    switch(type)
    {
        ...
        case PANEL_PIXMAP_BACKGROUND:
            ...
            if(panel_menu->bg_pixbuf) g_object_ref(G_OBJECT(panel_menu->bg_pixbuf));
            break;
        default:
            break;
    }
    if(GTK_WIDGET_MAPPED(panel_menu->menubar) && panel_menu->menubar->allocation.width > 1)
        panel_menu_setup_background(panel_menu);
}

static void
applet_size_allocate(GtkWidget *widget, GtkAllocation *allocation, PanelMenu *panel_menu)
{
    ...
    switch(panel_menu->bg_type)
    {
        ...
        case PANEL_PIXMAP_BACKGROUND:
            if(panel_menu->bg_pixbuf)
            {
                ...
                gint offset_x;
                gint offset_y;
                gint width = panel_menu->menubar->allocation.width;
                gint height = panel_menu->menubar->allocation.height;
                gint w = gdk_pixbuf_get_width(panel_menu->bg_pixbuf);
                gint h = gdk_pixbuf_get_height(panel_menu->bg_pixbuf);
                gint origin_x = 0;
                gint origin_y = 0;

                gtk_widget_translate_coordinates(panel_menu->menubar, gtk_widget_get_toplevel(panel_menu->menubar),
                                                 panel_menu->menubar->allocation.x, panel_menu->menubar->allocation.y,
                                                 &origin_x, &origin_y);
                panel_menu->pos_x = origin_x;
                panel_menu->pos_y = origin_y;
                if(origin_x > w) origin_x %= w;
                if(origin_y > h) origin_y %= h;
                g_print("origin x: %d origin y: %d\n", origin_x, origin_y);
                ...
                bgpixmap = gdk_pixmap_new(panel_menu->menubar->window, width, height, -1);
                gc = gdk_gc_new(GDK_DRAWABLE(bgpixmap));
                for(offset_y = 0; offset_y < height; offset_y += (h -  origin_y))
                {
                    if(offset_y > 0) origin_y = 0;
                    g_print("rendering from y value %d to %d\n", offset_y, offset_y + MIN(h - origin_y, height));
                    for(offset_x = 0; offset_x < width; offset_x += (w - origin_x))
                    {
                        if(offset_x > 0) origin_x = 0;
                        g_print("rendering from x value %d to %d\n", offset_x, offset_x + MIN(w - origin_x, width));
                        gdk_pixbuf_render_to_drawable(panel_menu->bg_pixbuf, GDK_DRAWABLE(bgpixmap), gc, 
                                                      origin_x, origin_y, offset_x, offset_y,
                                                      MIN(w - origin_x, width), MIN(h - origin_y, height),
                                                      GDK_RGB_DITHER_NONE, 0, 0);
                    }
                }
                g_object_unref(G_OBJECT(gc));
                new_style->bg_pixmap[GTK_STATE_NORMAL] = bgpixmap;
...
panel_menu_background_restore(PanelMenu *panel_menu)
{
    ...
    {
        gtk_widget_set_style(panel_menu->menubar, panel_menu->orig_style);
    ...
    {
        gtk_widget_modify_bg(panel_menu->menubar, GTK_STATE_NORMAL, &panel_menu->orig_bg_color);
...
static gboolean
panel_menu_drag_motion(GtkWidget *widget, GdkDragContext *context, gint x, gint y, guint time, PanelMenu *panel_menu)
{
    GList *list;
    GtkWidget *target;
    gint old_position;
    ...
    for(list = GTK_MENU_SHELL(widget)->children; list; list = list->next)
    {
        ...
        if(x >= target->allocation.x && x <= (target->allocation.x + target->allocation.width))
        {
            panel_menu->on_item = TRUE;
            gtk_draw_focus(widget->style, widget->window, target->allocation.x+1, target->allocation.y+1, target->allocation.width-2, target->allocation.height-2);
            break;
        }
        else if(x <= target->allocation.x) break;
        else panel_menu->position++;
    }
    if(panel_menu->position != old_position)
    {
        target = g_list_nth_data(GTK_MENU_SHELL(widget)->children, old_position);
        if(target) gtk_widget_queue_draw_area(widget, target->allocation.x, target->allocation.y, target->allocation.width, target->allocation.height);
    }
    return(FALSE);
}

static void
panel_menu_drag_leave(GtkWidget	*widget, GdkDragContext *context, guint time, PanelMenu *panel_menu)
{
    gtk_widget_queue_draw(widget);
}

static void
panel_menu_drag_data_received(GtkWidget *widget, GdkDragContext *context, gint x, gint y, GtkSelectionData *selection_data, guint info, guint time, PanelMenu *panel_menu)
{
    GList *list;
    GList *uris;
    GnomeVFSURI *uri;
    ...
    if(info == TARGET_URI_LIST)
    {
        ...
        if(!uris && selection_data->data)
        {
            ...
            if(entry && entry->type == PANEL_MENU_TYPE_ACTIONS)
                panel_menu_actions_accept_drop(entry, (gchar *)selection_data->data);
        }
...
panel_menu_accept_drop(PanelMenu *panel_menu, GnomeVFSURI *uri)
{
    ...
    if(!strncmp(fileuri, "applications:", strlen("applications:")))
    {
        if(strstr(fileuri, ".desktop"))
        {
            ...
            fileuri = gnome_vfs_uri_to_string(pathuri, GNOME_VFS_URI_HIDE_NONE);
        ...
        panel_menu->entries = g_list_insert(panel_menu->entries, (gpointer)entry, insert);
        panel_menu_options_insert_option(options, panel_menu_path_get_checkitem(entry), insert);
        gtk_menu_shell_insert(GTK_MENU_SHELL(panel_menu->menubar), panel_menu_path_get_widget(entry), insert);
        retval = TRUE;
...
panel_menu_check_entry_drop(PanelMenu *panel_menu, PanelMenuEntry *entry)
{
    ...
    if(entry->type == PANEL_MENU_TYPE_OPTIONS && entry->parent != panel_menu)
    {
        panel_menu = entry->parent;
        menuitem = panel_menu_common_get_entry_menuitem(entry);
        oldposition = (gint)g_object_get_data(G_OBJECT(menuitem), "old-position");
        gtk_menu_shell_insert(GTK_MENU_SHELL(panel_menu->menubar), menuitem, oldposition);
        g_object_unref(G_OBJECT(menuitem));
        panel_menu->entries = g_list_insert(panel_menu->entries, (gpointer)entry, oldposition);
        retval = FALSE;
    }
    return(retval);
}
...
panel_menu_accept_entry_drop(PanelMenu *panel_menu, PanelMenuEntry *entry)
{
    ...
    options = panel_menu_common_find_options(panel_menu);
    menuitem = panel_menu_common_get_entry_menuitem(entry);
    gtk_menu_shell_insert(GTK_MENU_SHELL(panel_menu->menubar), menuitem, panel_menu->position);
    g_object_unref(G_OBJECT(menuitem));
    if(entry->type != PANEL_MENU_TYPE_OPTIONS)
    {
        checkitem = panel_menu_common_get_entry_checkitem(entry);
        panel_menu_options_insert_option(options, checkitem, panel_menu->position);
        g_object_unref(G_OBJECT(checkitem));
    }
    panel_menu->entries = g_list_insert(panel_menu->entries, (gpointer)entry, panel_menu->position);
}
...
applet_destroy_cb(GtkWidget *widget, PanelMenu *panel_menu)
{
    ...
    if(panel_menu->profile_id) g_free(panel_menu->profile_id);
    if(panel_menu->applet_id) g_free(panel_menu->applet_id);

    if(panel_menu->icon_handle_image) g_free(panel_menu->icon_handle_image);
    for(cur = panel_menu->entries; cur; cur = cur->next)
    {
        entry = (PanelMenuEntry *)cur->data;
        panel_menu_common_call_entry_destroy(entry);
    }
    if(panel_menu->entries) g_list_free(panel_menu->entries);
    g_free(panel_menu);
}

static void
applet_change_orientation_cb(PanelApplet *applet, PanelAppletOrient  orient, PanelMenu *panel_menu)
{
...
applet_change_size_cb(PanelApplet  *applet, gint size, PanelMenu *panel_menu)
{
    if(panel_menu->size != size)
    {
        ...
        if(panel_menu->entries) panel_menu_options_rescale(panel_menu_common_find_options(panel_menu));
    }
}

static void
applet_help_cb(BonoboUIComponent *uic, PanelMenu *panel_menu, const gchar *verbname)
{
    /* FIXME: Implement this */
}

static void
applet_about_cb(BonoboUIComponent *uic, PanelMenu *panel_menu, const gchar *verbname)
{
    ...
    file = gnome_program_locate_file(NULL, GNOME_FILE_DOMAIN_PIXMAP, "panel-menu-icon.png", TRUE, NULL);
    if (!file)
    {
