...
static gint n_widget_drag_types = sizeof(widget_drag_types) / sizeof(GtkTargetEntry);

static void widget_dnd_drag_begin_cb(GtkWidget *widget, GdkDragContext *context);
static void widget_dnd_set_data_cb(GtkWidget *widget, GdkDragContext *context, GtkSelectionData *selection_data, guint info, guint time, PanelMenuEntry *entry);

static void apps_menuitem_dnd_drag_begin_cb(GtkWidget *widget, GdkDragContext *context, gpointer data);
static void apps_menuitem_dnd_set_data_cb(GtkWidget *widget, GdkDragContext *context, GtkSelectionData *selection_data, guint info, guint time, gpointer data);

static gboolean check_dir_exists(gchar *path);

static gint popup_menu_cb(GtkWidget *widget, GdkEventButton *event, PanelMenuEntry *entry);
static void remove_entry_cb(GtkWidget *widget, PanelMenuEntry *entry);
...
panel_menu_common_initialize()
{
    ...
    {
        gnome_vfs_make_directory(path, GNOME_VFS_PERM_USER_ALL | GNOME_VFS_PERM_GROUP_ALL | GNOME_VFS_PERM_OTHER_READ | GNOME_VFS_PERM_OTHER_EXEC);
    }
    g_free(path);
}
...
panel_menu_common_widget_dnd_init(PanelMenuEntry *entry)
{
	...
	gtk_drag_source_set(menuitem, GDK_BUTTON2_MASK, widget_drag_types, n_widget_drag_types, GDK_ACTION_COPY | GDK_ACTION_LINK | GDK_ACTION_ASK);
	g_signal_connect(G_OBJECT(menuitem), "drag_begin", G_CALLBACK(widget_dnd_drag_begin_cb), NULL);
	g_signal_connect(G_OBJECT(menuitem), "drag_data_get", G_CALLBACK(widget_dnd_set_data_cb), entry);
}
...
widget_dnd_drag_begin_cb(GtkWidget *widget, GdkDragContext *context)
{
    ...
    {
        if((image = gtk_image_menu_item_get_image(GTK_IMAGE_MENU_ITEM(widget))) && (pixbuf = gtk_image_get_pixbuf(GTK_IMAGE(image))))
            gtk_drag_set_icon_pixbuf(context, pixbuf, -5, -5);
    }
    else
    {
        window = gtk_window_new(GTK_WINDOW_POPUP);
        button = gtk_button_new_with_label(gtk_label_get_text(GTK_LABEL(GTK_BIN(widget)->child)));
...
static void
widget_dnd_set_data_cb(GtkWidget *widget, GdkDragContext *context, GtkSelectionData *selection_data, guint info, guint time, PanelMenuEntry *entry)
{
    ...
    if(info == TARGET_PANEL_MENU_ENTRY)
    {
        ...
        if(entry->type != PANEL_MENU_TYPE_OPTIONS)
        {
            ...
            gtk_container_remove(GTK_CONTAINER(checkitem->parent), checkitem);
        }
        else
        {
            g_object_set_data(G_OBJECT(menuitem), "old-position", GINT_TO_POINTER(position));
        }
        panel_menu->entries = g_list_remove(panel_menu->entries, entry);
        gtk_selection_data_set(selection_data, selection_data->target, 8, (gpointer)&entry, sizeof(gpointer));
    }
    else
    {
...
{
	gtk_drag_source_set(menuitem, GDK_BUTTON1_MASK, drag_types, n_drag_types, GDK_ACTION_COPY | GDK_ACTION_LINK | GDK_ACTION_ASK);
	g_signal_connect(G_OBJECT(menuitem), "drag_begin", G_CALLBACK(apps_menuitem_dnd_drag_begin_cb), NULL);
	g_signal_connect(G_OBJECT(menuitem), "drag_data_get", G_CALLBACK(apps_menuitem_dnd_set_data_cb), NULL);
}

static void
apps_menuitem_dnd_drag_begin_cb(GtkWidget *widget, GdkDragContext *context, gpointer data)
{
    GtkWidget *image;
    GdkPixbuf *pixbuf;
...
static void
apps_menuitem_dnd_set_data_cb(GtkWidget *widget, GdkDragContext *context, GtkSelectionData *selection_data, guint info, guint time, gpointer data)
{
    gchar *text = NULL;
...
void
panel_menu_common_set_icon_scaled_from_file(GtkMenuItem *menuitem, gchar *file)
{
    GdkPixbuf *pixbuf;
    GtkWidget *image;
    double pix_x, pix_y;
    ...
    if(!GTK_IS_IMAGE_MENU_ITEM(menuitem)) return;
...
void
panel_menu_common_set_visibility(GtkCheckMenuItem *checkitem, GtkWidget *target)
{
...
GtkWidget *
panel_menu_common_menuitem_from_path(gchar *uri, GtkMenuShell *parent, gboolean append)
{
    ...
    if(!append)
    {
        ...
        {
            if(g_object_get_data(G_OBJECT(cur->data), "append-before"))
            {
                break;
            }
            else if(g_object_get_data(G_OBJECT(cur->data), "append-after"))
            {
                position++;
                break;
            }
    ...
    item = panel_menu_desktop_item_load_uri((const char *)uri, "Application", TRUE);
    if(item && item->name && item->exec)
    {
        ...
        if(append) gtk_menu_shell_append(parent, menuitem);
        else gtk_menu_shell_insert(parent, menuitem, position);
        if(item->exec)
        {
            g_object_set_data(G_OBJECT(menuitem), "exec-string", g_strdup(item->exec));
    ...
    if(item) panel_menu_desktop_item_destroy(item);
    return(menuitem);
}

GtkWidget *
panel_menu_common_menu_from_path(gchar *name, gchar *subpath, GtkMenuShell *parent, gboolean append)
{
    GtkWidget *menuitem;
    GtkWidget *submenu;
    GtkWidget *tearoff;
...
panel_menu_common_find_options(PanelMenu *panel_menu)
{
    ...
    for(cur = panel_menu->entries; cur; cur = cur->next)
    {
        ...
        if(entry->type == PANEL_MENU_TYPE_OPTIONS) break;
    }
    return(entry);
}
...
GtkWidget *
panel_menu_common_get_entry_popup(PanelMenuEntry *entry)
{
    GtkWidget *menu = NULL;
    ...
    {
        case PANEL_MENU_TYPE_MENU_PATH:
            menu = panel_menu_path_get_popup(entry);
            break;
        case PANEL_MENU_TYPE_LINKS:
            menu = panel_menu_links_get_popup(entry);
            break;
        case PANEL_MENU_TYPE_DIRECTORY:
            menu = panel_menu_directory_get_popup(entry);
            break;
        case PANEL_MENU_TYPE_DOCUMENTS:
            menu = panel_menu_documents_get_popup(entry);
            break;
        case PANEL_MENU_TYPE_ACTIONS:
            menu = panel_menu_actions_get_popup(entry);
            break;
        case PANEL_MENU_TYPE_WINDOWS:
            menu = panel_menu_windows_get_popup(entry);
            break;
        case PANEL_MENU_TYPE_WORKSPACES:
            menu = panel_menu_workspaces_get_popup(entry);
            break;
        default:
            break;
    }
...
GtkWidget *
panel_menu_common_construct_entry_popup(PanelMenuEntry *entry)
{
    GtkWidget *menu;
    GtkWidget *menuitem;
    GtkWidget *image;
    ...
    menuitem = panel_menu_common_get_entry_menuitem(entry);
    g_signal_connect(G_OBJECT(menuitem), "button_press_event", G_CALLBACK(popup_menu_cb), entry);

    menu = gtk_menu_new();
    menuitem = gtk_image_menu_item_new_with_label("Remove");
    image = gtk_image_new_from_stock(GTK_STOCK_CLOSE, GTK_ICON_SIZE_MENU);
    gtk_image_menu_item_set_image(GTK_IMAGE_MENU_ITEM(menuitem), image);
    gtk_widget_show(image);
    gtk_menu_shell_append(GTK_MENU_SHELL(menu), menuitem);
    gtk_widget_show(menuitem);
    g_signal_connect(G_OBJECT(menuitem), "activate", G_CALLBACK(remove_entry_cb), entry);
...
static gint
popup_menu_cb(GtkWidget *widget, GdkEventButton *event, PanelMenuEntry *entry)
{
    GtkWidget *menu;
    gboolean handled = FALSE;

    menu = panel_menu_common_get_entry_popup(entry);
    ...
    if(handled) g_signal_stop_emission_by_name(widget, "button_press_event");
    return(handled);
}

static void
remove_entry_cb(GtkWidget *widget, PanelMenuEntry *entry)
{
    PanelMenu *panel_menu;
...
GtkWidget *
panel_menu_common_single_entry_dialog_new(gchar *title, gchar *label, gchar *value, GtkWidget **entry)
{
    GtkWidget *dialog;
    GtkWidget *box;
    GtkWidget *hbox;
