...
static gint panel_menu_documents_remove_cb(GtkWidget *widget, GdkEventKey *event, PanelMenuDocuments *documents);
...
panel_menu_documents_new(PanelMenu *parent, gchar *name)
{
    ...
    gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(documents->checkitem), TRUE);
    g_signal_connect(G_OBJECT(documents->checkitem), "toggled", G_CALLBACK(panel_menu_common_set_visibility), documents->documents);
    documents->menu = gtk_menu_new();
    tearoff = gtk_tearoff_menu_item_new();
    gtk_menu_shell_append(GTK_MENU_SHELL(documents->menu), tearoff);
    gtk_widget_show(tearoff);
    gtk_menu_item_set_submenu(GTK_MENU_ITEM(documents->documents), documents->menu);
    g_signal_connect(G_OBJECT(documents->menu), "key_press_event", G_CALLBACK(panel_menu_documents_remove_cb), documents);
    documents->popup = panel_menu_common_construct_entry_popup(entry);
    item = gtk_image_menu_item_new_with_label(_("Rename..."));
    image = gtk_image_new_from_stock(GTK_STOCK_CONVERT, GTK_ICON_SIZE_MENU);
    gtk_image_menu_item_set_image(GTK_IMAGE_MENU_ITEM(item), image);
    gtk_widget_show(image);
    gtk_menu_shell_append(GTK_MENU_SHELL(documents->popup), item);
    g_signal_connect(G_OBJECT(item), "activate", G_CALLBACK(rename_documents_cb), entry);
...
panel_menu_documents_set_name(PanelMenuEntry *entry, gchar *name)
{
    ...
    if(documents->name) g_free(documents->name);
    documents->name = name ? g_strdup(name) : g_strdup("");
    gtk_label_set_text(GTK_LABEL(GTK_BIN(documents->documents)->child), name);
    show_label = g_strconcat("Show ", documents->name, NULL);
    gtk_label_set_text(GTK_LABEL(GTK_BIN(documents->checkitem)->child), show_label);
    g_free(show_label);
}
...
panel_menu_documents_destroy(PanelMenuEntry *entry)
{
    ...
    documents = (PanelMenuDocuments *)entry->data;
    if(documents->name) g_free(documents->name);
    if(documents->items_list) g_list_free(documents->items_list);
...
GtkWidget *
panel_menu_documents_get_popup(PanelMenuEntry *entry)
{
    PanelMenuDocuments *documents;
    ...
    if(!strncmp(uri, "file:", strlen("file:")))
    {
        if(gnome_vfs_get_file_info(uri, &finfo, GNOME_VFS_FILE_INFO_FOLLOW_LINKS) == GNOME_VFS_OK)
        {
            if(finfo.type == GNOME_VFS_FILE_TYPE_REGULAR)
            {
                ...
                gtk_menu_shell_append(GTK_MENU_SHELL(documents->menu), menuitem);
                ...
                documents->items_list = g_list_append(documents->items_list, menuitem);
                documents->docs_list = g_list_append(documents->docs_list, g_strdup(uri));
                retval = TRUE;
            }
        }
    }
...
static gint
panel_menu_documents_remove_cb(GtkWidget *widget, GdkEventKey *event, PanelMenuDocuments *documents)
{
    GtkWidget *item;
    GList *cur;
    gchar *string;
    ...
    if(event->keyval == GDK_Delete)
    {
        ...
        for(cur = documents->items_list; cur; cur = cur->next, counter++)
        {
            if(item == GTK_WIDGET(cur->data))
            {
                documents->items_list = g_list_remove(documents->items_list, cur->data);
                temp = (gchar *)g_list_nth_data(documents->docs_list, counter);
                g_print("removing %s from the documents list\n", temp);
                documents->docs_list = g_list_remove(documents->docs_list, temp);
...
panel_menu_documents_new_with_dialog(PanelMenu *panel_menu)
{
    ...
    dialog = panel_menu_common_single_entry_dialog_new(_("Create documents item..."), _("Name:"), _("Documents"), &name_entry);
    ...
    if(response == GTK_RESPONSE_OK)
    {
        ...
        panel_menu_options_append_option(panel_menu_common_find_options(panel_menu), panel_menu_documents_get_checkitem(entry));
...
rename_documents_cb(GtkWidget *widget, PanelMenuEntry *entry)
{
    ...
    dialog = panel_menu_common_single_entry_dialog_new(_("Rename documents item..."), _("Name:"), documents->name, &name_entry);
    gtk_widget_show(dialog);
    gtk_widget_grab_focus(name_entry);
