...
static void rename_actions_cb(GtkWidget *widget, PanelMenuEntry *entry);
static void add_actions_cb(GtkWidget *widget, PanelMenuEntry *entry);
static void handle_response(GtkWidget *dialog, gint response, gpointer data);
static void button_clicked_append_item(GtkWidget *button, PanelMenuEntry *entry);
static void action_item_dnd_init(GtkWidget *button);
static void action_item_dnd_drag_begin_cb(GtkWidget *button, GdkDragContext *context);
static void action_item_dnd_set_data_cb(GtkWidget *button, GdkDragContext *context, GtkSelectionData *selection_data, guint info, guint time);
...
{
    { ACTION_ITEM_TYPE_RUN, "Run...", "action:run", DATADIR "/pixmaps/gnome-run.png", construct_run_item },
    { ACTION_ITEM_TYPE_LOGOUT, "Logout", "action:logout", DATADIR "/pixmaps/gnome-term-night.png", construct_logout_item },
    { ACTION_ITEM_TYPE_LOCK, "Lock Screen", "action:lock", DATADIR "/pixmaps/gnome-lockscreen.png", construct_lock_item },
    { ACTION_ITEM_TYPE_SHOW_DESKTOP, "Show Desktop", "action:show-desktop", DATADIR "/pixmaps/gnome-ccdesktop.png", construct_show_desktop_item }
};

static gint actions = sizeof(action_items) / sizeof(ActionItem);
...
panel_menu_actions_new(PanelMenu *parent, gchar *name)
{
    ...
    gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(actions->checkitem), TRUE);
    g_signal_connect(G_OBJECT(actions->checkitem), "toggled", G_CALLBACK(panel_menu_common_set_visibility), actions->actions);
    actions->menu = gtk_menu_new();
    tearoff = gtk_tearoff_menu_item_new();
    gtk_menu_shell_append(GTK_MENU_SHELL(actions->menu), tearoff);
    gtk_widget_show(tearoff);
    gtk_menu_item_set_submenu(GTK_MENU_ITEM(actions->actions), actions->menu);
    actions->popup = panel_menu_common_construct_entry_popup(entry);
    item = gtk_image_menu_item_new_with_label(_("Add actions..."));
    image = gtk_image_new_from_stock(GTK_STOCK_ADD, GTK_ICON_SIZE_MENU);
    gtk_image_menu_item_set_image(GTK_IMAGE_MENU_ITEM(item), image);
    gtk_widget_show(image);
    gtk_menu_shell_append(GTK_MENU_SHELL(actions->popup), item);
    g_signal_connect(G_OBJECT(item), "activate", G_CALLBACK(add_actions_cb), entry);
    ...
    gtk_widget_show(image);
    gtk_menu_shell_append(GTK_MENU_SHELL(actions->popup), item);
    g_signal_connect(G_OBJECT(item), "activate", G_CALLBACK(rename_actions_cb), entry);
    gtk_widget_show(item);
    return(entry);
}

void
panel_menu_actions_set_name(PanelMenuEntry *entry, gchar *name)
{
    PanelMenuActions *actions;
    gchar *show_label;
    ...
    if(actions->name) g_free(actions->name);
    actions->name = name ? g_strdup(name) : g_strdup("");
    gtk_label_set_text(GTK_LABEL(GTK_BIN(actions->actions)->child), name);
    show_label = g_strconcat("Show ", actions->name, NULL);
    gtk_label_set_text(GTK_LABEL(GTK_BIN(actions->checkitem)->child), show_label);
    g_free(show_label);
}
...
GtkWidget *
panel_menu_actions_get_popup(PanelMenuEntry *entry)
{
    PanelMenuActions *actions;
...
panel_menu_actions_append_item(PanelMenuEntry *entry, gchar *uri)
{
    ...
    if(!strncmp(uri, "action:", strlen("action:")))
    {
        ...
        {
            gtk_menu_shell_append(GTK_MENU_SHELL(actions->menu), menuitem);
            /* action_item_dnd_init(menuitem); */
            actions->items_list = g_list_append(actions->items_list, menuitem);
            actions->actions_list = g_list_append(actions->actions_list, g_strdup(uri));
            retval = TRUE;
        }
    }
    return(retval);
...
check_valid_uri(gchar *uri)
{
    ...
    for(counter = 0; counter < actions; counter++)
    {
...
construct_from_uri(gchar *uri)
{
    ...
    for(counter = 0; counter < actions; counter++)
    {
        if(!strcmp(uri, action_items[counter].path))
        {
            widget = gtk_image_menu_item_new_with_label(action_items[counter].name);
            panel_menu_common_set_icon_scaled_from_file(GTK_MENU_ITEM(widget), action_items[counter].image);
            g_object_set_data(G_OBJECT(widget), "uri-path", action_items[counter].path);
...
{
    g_signal_connect(G_OBJECT(widget), "activate", G_CALLBACK(show_desktop_cb), NULL);
}
...
show_desktop_cb(GtkWidget *widget, gpointer data)
{
    ...
    for(windows = wnck_screen_get_windows(wnck_screen_get(0)); windows; windows = windows->next)
    {
        win = (WnckWindow *)windows->data;
        if((active == wnck_window_get_workspace(win)) && !wnck_window_is_minimized(win))
        {
...
panel_menu_actions_new_with_dialog(PanelMenu *panel_menu)
{
    ...
    dialog = panel_menu_common_single_entry_dialog_new(_("Create actions item..."), _("Name:"), _("Actions"), &name_entry);
    ...
    if(response == GTK_RESPONSE_OK)
    {
        ...
        panel_menu_options_append_option(panel_menu_common_find_options(panel_menu), panel_menu_actions_get_checkitem(entry));
...
static void
rename_actions_cb(GtkWidget *widget, PanelMenuEntry *entry)
{
    PanelMenuActions *actions;
    GtkWidget *dialog;
    GtkWidget *box;
    ...
    dialog = panel_menu_common_single_entry_dialog_new(_("Rename actions item..."), _("Name:"), actions->name, &name_entry);
    gtk_widget_show(dialog);
    gtk_widget_grab_focus(name_entry);
...
static void
add_actions_cb(GtkWidget *widget, PanelMenuEntry *entry)
{
    GtkWidget *dialog;
    GtkWidget *box;
    GtkWidget *bbox;
    ...
    dialog = gtk_dialog_new_with_buttons(_("Add items to actions menu"),
                                         NULL, GTK_DIALOG_MODAL,
                                         GTK_STOCK_CLOSE, GTK_RESPONSE_CLOSE, NULL);
    box = GTK_DIALOG(dialog)->vbox;
    gtk_container_set_border_width(GTK_CONTAINER(box), 5);

    label = gtk_label_new("To add an item to your actions menu,\n"
    ...
    for(counter = 0; counter < actions; counter++)
    {
        ...
        pixbuf = gdk_pixbuf_new_from_file(action_items[counter].image, NULL);
        if(pixbuf)
        {
        ...
        gtk_widget_show(label);
        g_object_set_data(G_OBJECT(button), "uri-path", action_items[counter].path);
        g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(button_clicked_append_item), entry);
        action_item_dnd_init(button);
        gtk_box_pack_start(GTK_BOX(bbox), button, FALSE, FALSE, 0);
        gtk_widget_show(button);
    }
    gtk_widget_show(dialog);
    g_signal_connect(G_OBJECT(dialog), "response", G_CALLBACK(handle_response), NULL);
}

static void
handle_response(GtkWidget *dialog, gint response, gpointer data)
{
    gtk_widget_destroy(dialog);
}
...
{
	gtk_drag_source_set(button, GDK_BUTTON2_MASK, drag_types, n_drag_types, GDK_ACTION_COPY | GDK_ACTION_LINK | GDK_ACTION_ASK);
	g_signal_connect(G_OBJECT(button), "drag_begin", G_CALLBACK(action_item_dnd_drag_begin_cb), NULL);
	g_signal_connect(G_OBJECT(button), "drag_data_get", G_CALLBACK(action_item_dnd_set_data_cb), NULL);
}
...
action_item_dnd_drag_begin_cb(GtkWidget *button, GdkDragContext *context)
{
    ...
    {
        if((image = gtk_image_menu_item_get_image(GTK_IMAGE_MENU_ITEM(button))) && (pixbuf = gtk_image_get_pixbuf(GTK_IMAGE(image))))
    ...
    else if(GTK_IS_BUTTON(button))
    {
        ...
        gtk_widget_show(box);
        for(children = GTK_BOX(GTK_BIN(button)->child)->children; children; children = children->next)
        {
            temp = NULL;
            widget = ((GtkBoxChild *)(children->data))->widget;
            if(GTK_IS_LABEL(widget))
                temp = gtk_label_new(gtk_label_get_text(GTK_LABEL(widget)));
            else if(GTK_IS_IMAGE(widget))
                temp = gtk_image_new_from_pixbuf(gtk_image_get_pixbuf(GTK_IMAGE(widget)));
            if(temp) gtk_box_pack_start(GTK_BOX(box), temp, FALSE, FALSE, 0);
        }
...
static void
action_item_dnd_set_data_cb(GtkWidget *button, GdkDragContext *context, GtkSelectionData *selection_data, guint info, guint time)
{
    gchar *uri;

    if((uri = g_object_get_data(G_OBJECT(button), "uri-path")))
    {
        gtk_selection_data_set(selection_data, selection_data->target, 8, uri, strlen(uri));
    }
    else
    {
