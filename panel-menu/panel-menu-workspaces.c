...
static void wnck_active_workspace_changed(WnckScreen *screen, GtkMenuShell *menu);
static void wnck_workspace_added(WnckScreen *screen, WnckWorkspace *workspace, GtkMenuShell *menu);
static void wnck_workspace_removed(WnckScreen *screen, WnckWorkspace *workspace, GtkMenuShell *menu);
...
panel_menu_workspaces_new(PanelMenu *parent)
{
    ...
    workspaces->checkitem = gtk_check_menu_item_new_with_label(_("Show Workspaces"));
    gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(workspaces->checkitem), TRUE);
    g_signal_connect(G_OBJECT(workspaces->checkitem), "toggled", G_CALLBACK(panel_menu_common_set_visibility), workspaces->workspaces);
    workspaces->menu = gtk_menu_new();
    tearoff = gtk_tearoff_menu_item_new();
    gtk_menu_shell_append(GTK_MENU_SHELL(workspaces->menu), tearoff);
    gtk_widget_show(tearoff);
    gtk_menu_item_set_submenu(GTK_MENU_ITEM(workspaces->workspaces), workspaces->menu);
    workspaces->popup = panel_menu_common_construct_entry_popup(entry);
...
panel_menu_workspaces_destroy(PanelMenuEntry *entry)
{
    ...
    g_signal_handler_disconnect(workspaces->screen, workspaces->active_changed);
    g_signal_handler_disconnect(workspaces->screen, workspaces->workspace_created);
    g_signal_handler_disconnect(workspaces->screen, workspaces->workspace_destroyed);
...
GtkWidget *
panel_menu_workspaces_get_popup(PanelMenuEntry *entry)
{
    PanelMenuWorkspaces *workspaces;
...
fill_workspace_menu(GtkMenuShell *menu)
{
    ...
    for(count = 0; count < spaces; count++)
    {
        ...
        if(active == workspace) gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(menuitem), TRUE);
        gtk_widget_show(menuitem);
        g_free(string);
        gtk_menu_shell_append(menu, menuitem);
        g_signal_connect(G_OBJECT(menuitem), "activate", G_CALLBACK(handle_workspace_change), GINT_TO_POINTER(count));
...
handle_workspace_change(GtkWidget *menuitem, int number)
{
    ...
    if(workspace && workspace != active && GTK_CHECK_MENU_ITEM(menuitem)->active)
    {
...
setup_workspace_signals(PanelMenuWorkspaces *workspaces)
{
    ...
    workspaces->active_changed = g_signal_connect(G_OBJECT(screen), "active_workspace_changed", G_CALLBACK(wnck_active_workspace_changed), workspaces->menu);
    workspaces->workspace_created = g_signal_connect(G_OBJECT(screen), "workspace_created", G_CALLBACK(wnck_workspace_added), workspaces->menu);
    workspaces->workspace_destroyed = g_signal_connect(G_OBJECT(screen), "workspace_destroyed", G_CALLBACK(wnck_workspace_removed), workspaces->menu);
}
...
wnck_active_workspace_changed(WnckScreen *screen, GtkMenuShell *menu)
{
    ...
    gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(g_list_nth_data(menu->children, active_num+1)), TRUE);
}

static void
wnck_workspace_added(WnckScreen *screen, WnckWorkspace *workspace, GtkMenuShell *menu)
{
    ...
    if(menu->children &&  (g_list_length(menu->children) > 1) && (menuitem = g_list_nth_data(menu->children, g_list_length(menu->children)-1)))
    {
        group = GTK_RADIO_MENU_ITEM(menuitem)->group;
    }
    active = wnck_screen_get_active_workspace(screen);
...
static void
wnck_workspace_removed(WnckScreen *screen, WnckWorkspace *workspace, GtkMenuShell *menu)
{
