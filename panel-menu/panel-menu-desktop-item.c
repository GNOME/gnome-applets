...
panel_menu_desktop_item_get_type (void)
{
    ...
    if (type == 0)
    {
        type = g_boxed_type_register_static ("PanelMenuDesktopItem", _panel_menu_desktop_item_copy, _panel_menu_desktop_item_free);
    }
    return(type);
}
...
panel_menu_desktop_item_load_file (const char *file, const char *expected_type, gboolean    run_tryexec)
{
    ...
    retval = panel_menu_desktop_item_load_uri (uri, expected_type, run_tryexec);
...
static const int
guess_encoding (const char *uri, const char *name, const char *comment, gboolean old_kde)
{
    ...
    else if (g_utf8_validate (name, -1, NULL) && (comment == NULL || g_utf8_validate (comment, -1, NULL)))
    {
...
PanelMenuDesktopItem *
panel_menu_desktop_item_load_uri (const char *uri, const char *expected_type, gboolean run_tryexec)
{
    ...
    while (readbuf_gets (buf, sizeof (buf), rb) != NULL)
    {
        ...
        {
            if (expected_type != NULL && strcmp (val, expected_type) != 0)
            {
    ...
    if (retval != NULL && (name_lang != NULL || comment_lang != NULL) && encoding == ENCODING_UNKNOWN)
    {
        encoding = guess_encoding (uri, retval->name, retval->comment, old_kde);
    }

    if (retval != NULL && name_lang != NULL)
    {
        const char *enc = get_encoding (encoding, name_lang);
        char *utf8_string = g_convert (retval->name, -1, "UTF-8", enc, NULL, NULL, NULL);
        if (utf8_string != NULL)
        {
            g_free (retval->name);
    ...
    if (retval != NULL && retval->comment != NULL && comment_lang != NULL)
    {
        ...
        char *utf8_string = g_convert (retval->comment, -1, "UTF-8", enc, NULL, NULL, NULL);
        if (utf8_string != NULL)
        {
            g_free (retval->comment);
...
panel_menu_desktop_item_find_icon (const char *icon)
{
    ...
    else
    {
        ...
        full = gnome_program_locate_file (NULL, GNOME_FILE_DOMAIN_PIXMAP, icon, TRUE, NULL);
        if (full == NULL)
            full = gnome_program_locate_file (NULL, GNOME_FILE_DOMAIN_APP_PIXMAP, icon, TRUE, NULL);
        if (kde_dirs == NULL)
...
readbuf_getc (ReadBuf *rb)
{
    ...
    if (rb->size == 0 ||
        ...
        if (gnome_vfs_read (rb->handle, rb->buf, BUFSIZ, &bytes_read) != GNOME_VFS_OK)
        {
            rb->eof = TRUE;
            return EOF;
        }
...
{
    if (gnome_vfs_seek (rb->handle, GNOME_VFS_SEEK_START, 0) == GNOME_VFS_OK)
        return TRUE;

    gnome_vfs_close (rb->handle);
    rb->handle = NULL;
    if (gnome_vfs_open (&rb->handle, rb->uri, GNOME_VFS_OPEN_READ) == GNOME_VFS_OK)
        return TRUE;
    return FALSE;
}
