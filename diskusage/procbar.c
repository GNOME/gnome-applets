...
ProcBar *
procbar_new (GtkWidget *label, gint orient, gint n, GdkColor *colors, gint (*cb)())
{
	ProcBar *pb;

	/*printf ("entering procbar_new \n");*/

	pb = g_new (ProcBar, 1);

	pb->orientation = orient;
	...
	if (pb->orientation == PROCBAR_HORIZONTAL) {
		...
		pb->vbox = gtk_vbox_new (FALSE, GNOME_PAD_SMALL);
		pb->box = pb->vbox;
	}
	...
	if (pb->orientation == PROCBAR_HORIZONTAL) {
		...
		if (label)
			gtk_box_pack_start (GTK_BOX (pb->vbox), label, FALSE, TRUE, 0);
		gtk_box_pack_start_defaults (GTK_BOX (pb->vbox), pb->frame);
	}
	...
	if (pb->orientation == PROCBAR_HORIZONTAL) 
		...
		gtk_widget_show (pb->vbox);
	
	/*printf ("leaving procbar_new \n");*/
...
procbar_set_values (ProcBar *pb, unsigned val [])
{
	...
	if (pb->orientation == PROCBAR_HORIZONTAL) {
		...
		for (i=0; i<pb->n; i++) {
			if (i<pb->n-1)
				hr = (unsigned) h * ((float)val [i+1]/tot);
			else
				hr = A.height - y;
