...
{
	if (id->end_func) id->end_func(id->module, id->data, id, ad);
...
static gint draw_display_line(AppData *ad)
{
	...
	if (ad->x_pos == 0)
		...
		if (ad->y_pos == 0)
			{
			if (id->pre_func) id->pre_func(id->module, id->data, id, ad);
...
static int display_click(GtkWidget *w, GdkEventButton *event, gpointer data)
{
	...
	while(list)
		...
		if (x >= cd->x && x < cd->x + cd->w && y >= cd->y && y < cd->y + cd->h)
			{
			if (cd->click_func) cd->click_func(cd->line_id->module, cd->line_id->data, cd->line_id, ad);
			}
